...
export default async function ClientThreadPage(props: { params: Promise<{ threadId: string }> }) {
  const { threadId } = await props.params;

  const session = await safeGetServerSession();
  if (!session) {
    redirect(`/login?next=${encodeURIComponent(`/dashboard/client/threads/${threadId}`)}`);
  }
  ...
  if (!type) {
    redirect(`/login?error=no_account&next=${encodeURIComponent(`/dashboard/client/threads/${threadId}`)}`);
  }
  if (String((acct as any)?.status || "") === "disabled") {
    redirect(`/login?error=disabled&next=${encodeURIComponent(`/dashboard/client/threads/${threadId}`)}`);
  }
  ...
  async function markThreadRead(formData: FormData) {
    ...
    const thread = await writeClient.fetch(
      `*[_type == "messageThread" && _id == $id][0]{
        ...
        "messages": messages[visibility == "client"]{createdAt}
      }`,
      { id: threadId },
    );
    if (!thread?._id) return;
    if (String(thread.visibility || "") !== "client") return;
    ...
    redirect(`/dashboard/client/threads/${threadId}`);
  }
  ...
  if (String(thread?.visibility || "") !== "client") {
