...
import { markThreadRead, postThreadMessage, togglePinMessage } from "../actions";

export const dynamic = "force-dynamic";

export default async function AdminThreadPage(props: { params: Promise<{ threadId: string }> }) {
  const { threadId } = await props.params;

  const session = await safeGetServerSession();
  if (!session) {
    redirect(`/login?next=${encodeURIComponent(`/dashboard/admin/threads/${threadId}`)}`);
  }
  ...
  if (!type) {
    redirect(`/login?error=no_account&next=${encodeURIComponent(`/dashboard/admin/threads/${threadId}`)}`);
  }
  if (String((acct as any)?.status || "") === "disabled") {
    redirect(`/login?error=disabled&next=${encodeURIComponent(`/dashboard/admin/threads/${threadId}`)}`);
  }
  if (type !== "admin") {
    redirect("/dashboard");
  }
  ...
  const canMarkRead = hasAccountCapability(acct, "message.read");
  const canPin = hasAccountCapability(acct, "message.pin");
  ...
  const canView =
    ...
    participants.some((p) => String(p?._id || "") === String(acct?._id || ""));
