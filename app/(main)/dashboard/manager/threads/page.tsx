...
export default async function ManagerThreadsPage() {
  const session = await safeGetServerSession();
  if (!session) redirect("/login");

  const email = String((session as any)?.user?.email || "");
  const { data: currentUser } = await sanityFetch({
    query: `*[_type == "account" && email == $email][0]{_id, type}`,
    params: { email },
  });

  if (!currentUser || currentUser.type !== "manager") redirect("/dashboard");
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-4">
        ...
        {(threads as any[])?.map((thread) => (
          <Link key={thread._id} href={`/dashboard/manager/threads/${thread._id}`}>
