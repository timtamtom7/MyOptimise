...
import { TaskWithDetails, TaskStatus, TaskPriority, TaskVisibility } from '../../../lib/supabase'
...
export default function TaskList({ userId, onTaskSelect }: TaskListProps) {
  ...
  useEffect(() => {
    const initTaskService = async () => {
      try {
        const service = await createTaskService(userId)
