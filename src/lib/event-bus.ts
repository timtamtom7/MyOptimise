...
export class EventBus {
  ...
  private notifySubscribers(event: EventData): void {
    this.subscriptions.forEach((subscription) => {
      ...
      const matchesOrganization = !subscription.organizationId || subscription.organizationId === event.organizationId
      const matchesUser = !subscription.userId || subscription.userId === event.userId

      if (matchesEventType && matchesOrganization && matchesUser) {
        try {
  ...
  async emit(eventType: string, payload: any, organizationId: string, userId: string, metadata?: Record<string, any>): Promise<void> {
    try {
  ...
  subscribe(eventTypes: string[], callback: (event: EventData) => void, organizationId?: string, userId?: string): string {
...
export const createTaskEvent = (taskId: string, action: string, organizationId: string, userId: string, details?: any) => {
  return {
    type: `task_${action}`,
    payload: {
      task_id: taskId,
...
export const createCalendarEvent = (eventId: string, action: string, organizationId: string, userId: string, details?: any) => {
  return {
...
export const createMessageEvent = (messageId: string, threadId: string, action: string, organizationId: string, userId: string, details?: any) => {
  return {
...
export const createAnalyticsEvent = (serviceId: string, metricName: string, value: number, organizationId: string, userId: string, details?: any) => {
  return {
