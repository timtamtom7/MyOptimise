...
export class TaskService {
  ...
  constructor(userId: string, organizationId: string, capabilities: Capability[]) {
    this.userId = userId
...
export const createTaskService = async (userId: string): Promise<TaskService> => {
  ...
  const capabilityList = capabilities.data?.map(uc => uc.capability) || []

  return new TaskService(userId, user.organization_id, capabilityList)
}