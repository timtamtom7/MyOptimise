...
describe('TaskService', () => {
  ...
  const accountId = 'org-123'
  const capabilities = ['manage_tasks', 'view_analytics']

  beforeEach(() => {
    taskService = new TaskService(userId, accountId, capabilities)
    vi.clearAllMocks()
  })
  ...
  describe('createTask', () => {
    ...
    it('should throw error if user lacks manage_tasks capability', async () => {
      const taskServiceNoCapability = new TaskService(userId, accountId, ['view_analytics'])
