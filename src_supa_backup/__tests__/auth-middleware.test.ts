...
describe('AuthMiddleware', () => {
  ...
  describe('authenticateRequest', () => {
    ...
    it('should return authenticated user for valid token', async () => {
      ...
      expect(user?.accountId).toBe('org-123')
    })
  })
  ...
  describe('authorize', () => {
    ...
    it('should require account membership when specified', () => {
      const user = {
        id: 'user-123',
        email: 'test@example.com',
        fullName: 'Test User',
      ...
      const result = middleware.authorize(user, config)
      expect(result.authorized).toBe(false)
      expect(result.reason).toContain('Account membership required')
    })
  })
  ...
  describe('requireAuth', () => {
    ...
    it('should allow authorized requests and set headers', async () => {
      ...
      const response = await middleware.requireAuth(request, { allowEmployeeAccess: true })
