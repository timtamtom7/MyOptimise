...
import { supabase, getCurrentUser, UserWithCapabilities, Capability } from '@/lib/supabase'
...
export class AuthMiddleware {
  ...
  async authenticateRequest(request: NextRequest): Promise<AuthUser | null> {
    try {
      ...
      return {
        ...
        accountId: currentUser.organization_id,
  ...
  canAccessAccount(user: AuthUser, accountId: string): boolean {
    return user.accountId === accountId
  }

  authorize(user: AuthUser, config: RouteConfig): { authorized: boolean; reason?: string } {
    if (config.requireAccount && !user.accountId) {
      return { authorized: false, reason: 'Account membership required' }
    }
  ...
  async requireAuth(request: NextRequest, config: RouteConfig = {}): Promise<NextResponse> {
    ...
    response.headers.set('x-organization-id', user.accountId)
